  ...
  <section class="graphs">
    <header class="graphs__header">
      <div>
        ...
        <p>Serie temporal de la componente H medida por la estación seleccionada.</p>
      </div>
      <form class="graphs__controls" @submit.prevent>
        <label class="control">
          <span>Estación</span>
          <select v-model="station">
            <option v-for="option in stationOptions" :key="option" :value="option">
              {{ option }}
            </option>
          </select>
        </label>
    ...
    <section class="graphs__body" aria-live="polite">
      ...
      <figure v-else class="chart-wrapper">
        ...
        <figcaption class="chart-meta">
          ...
          <div>
            <span>Mínimo</span>
            <strong>{{ minValue.toFixed(1) }} {{ unit }}</strong>
          </div>
          <div>
...
const stationOptions = ['CHI', 'BOG', 'LAP', 'HON'];
...
const fetchData = async () => {
  ...
  try {
    ...
    seriesData.value = payload.series?.[0]?.data ?? [];
  } catch (error) {
...
watch([station, range, every, unit], () => {
  fetchData();
});
...
const yTicks = computed(() => {
  if (!hasData.value) return [];
  const divisions = 4;
  const step = valueRange.value / divisions;
  return Array.from({ length: divisions + 1 }, (_, index) => {
    const value = minValue.value + step * index;
    const normalized = (value - minValue.value) / valueRange.value;
    const y = chartHeight - padding - normalized * chartInnerHeight.value;
    return {
      y,
